    ...
    public function storeAssignment($request) {
        if(Auth::user()->hasPermissionTo('Create Assignment')) {
            if(Auth::user()->hasRole('Teacher')) {
                ...
                $assignment->start_date_time = $request->start_date_time;
                $assignment->end_date_time = $request->end_date_time;
                ...
                $new_path = Storage::disk('s3')->url($path);
                $assignment->file_link = $new_path;
                if(true) {
                    $assignment->save();
